...
```{r}
# Base map - what all plots have in common
base_rth <- ggplot(cond_evidence_by_week, aes(x = week, y = n, color = evidence_category)) +
...
We could also add in smoothed means:

```{r}
base_rth +
  geom_point() +
  geom_smooth(se = FALSE, lwd = 0.8)
```

Or just do a line graph:
```{r}
base_rth +
  geom_line()
```
