...
```{r}
gadwall <- ggplot(filter(breeding_evidence, Common.Name == "Gadwall"), aes(x = week, color = evidence_category, fill = evidence_category)) +
    geom_density(adjust=1.5, alpha=.3) +
    labs(title = "Breeding Phenology", 
       subtitle = "Gadwall") +
    labs(x = "Week",
...
```{r eval = FALSE}
grid.arrange(mallard, gadwall, ncol=2)
```
