...
The code below is very repetitive so should not be seen as an example of good coding. It was just the fastest way to get some graphs to show to Mark. I'll work on streamlining the code.
...
THIS IS NOT AN EFFICIENT WAY OF PULLING THIS DATA TOGETHER - SEE BELOW UNDER BOXPLOTS FOR A MORE PROMISING AVENUE
...
Still working on this, but the next code chunk is a better way to pull all the relevant data for a number of species together, instead of doing it separately for each species.
...
```{r}
ggplot(filter(breeding_evidence, Common.Name == "Red-tailed Hawk"), aes(x = week, color = evidence_category, fill = evidence_category)) +
